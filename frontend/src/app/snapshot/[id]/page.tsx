...
function isoToMonth(iso: string) {
  return iso.slice(0, 7); // "2024-03-01" → "2024-03"
}
...
export default function SnapshotEditPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API_URL}/api/v1/snapshots/${id}`).then((r) => r.json()) as Promise<Snapshot>,
      fetch(`${API_URL}/api/v1/user-items/`).then((r) => r.json()) as Promise<UserItem[]>,
    ])
      .then(([snapshot, userItems]) => {
        setMonth(isoToMonth(snapshot.snapshot_month));
        setItems(userItems);

        const prefilled: Record<string, string> = {};
        for (const item of userItems) {
          const [section, subcategory] = item.category.split(".");
          const sectionData = snapshot.data[section as keyof SnapshotData];
          const val = sectionData?.[subcategory]?.[item.id];
          if (val) prefilled[item.id] = String(val);
        }
  ...
  function handleAmountChange(itemId: string, value: string) {
  ...
  async function handleDeleteItem(itemId: string) {
    try {
      await fetch(`${API_URL}/api/v1/user-items/${itemId}`, { method: "DELETE" });
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">스냅샷 수정</h2>
          <p className="mt-1 text-sm text-gray-500">{month.replace("-", "년 ")}월 스냅샷</p>
        </div>
        <button
          type="button"
